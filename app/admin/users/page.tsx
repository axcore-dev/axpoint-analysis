...
const COLUMNS = ["이메일", "이름", "로그인 방식", "권한", "상태", "가입일"];

/** 사용자 관리 — 화면 골격. 데이터·동작은 API 연동 전까지 팝업 안내. */
    ...
    <section style={{ maxWidth: 1040 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        <div style={{ flex: "1 1 280px", maxWidth: 420 }} onClick={open}>
          <Input placeholder="이메일 또는 이름 검색" readOnly aria-label="사용자 검색" />
        </div>
        <Button variant="utility" size="md" onClick={open}>
          <Icons.chevronDown size={15} aria-hidden /> 권한
        </Button>
        <Button variant="utility" size="md" onClick={open}>
